...
const hash = async (SOLA_FILE_PATH, SOLA_HASH_PATH, relativePath) => {
    ...
    const xmlZipFileDestinationFolder = path.dirname(path.join(SOLA_HASH_PATH, relativePath));
    ...
    const {stderr: ffmpegLog} = child_process.spawnSync(
        ...
        [
            ...
            "-an",
            "-vf",
            `"fps=fps=2,scale=-1:144,showinfo"`,
            //`"select=eq(pict_type\\,PICT_TYPE_I),scale=1:144,showinfo"`,
            `-vsync`,
            //0,
            `2`,
            //'-copyts',
        ...
        {encoding: "utf-8", maxBuffer: 1024 * 1024 * 100, shell: true}
    );
    ...
    if (thumbnailList.length !== timeCodeList.length)
    {
        console.log(`${relativePath} parsed non-matching frames and timecodes. This is a VFR vsync error. Continuing to Next File`);
        return false;
    }
    ...
    fs.ensureDirSync(xmlZipFileDestinationFolder);
